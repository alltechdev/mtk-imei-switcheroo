...
3. **WiFi runtime byte-level confirmation**: after a separate `live_patch_mac.sh` run that wrote `02:de:ad:be:ef:22` to `WIFI[4:10]`, boot logcat shows `wpa_supplicant: p2p0: Own MAC address: 02:de:ad:be:ef:22` — `p2p0`'s Own MAC is brought up directly from the chipset's permanent MAC (no Android randomization in between, unlike `wlan0`), so it matching the patched WIFI file proves the kernel WiFi driver loaded the patched bytes at boot.
