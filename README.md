...
- **TIQ M5** — partial. **BT MAC: `live_patch_mac.sh` works end-to-end** on hardware (file persists byte-identical, daemon validates, `settings get secure bluetooth_address` returns the patched value byte-for-byte across reboots). **WiFi MAC: file-level only.** `live_patch_mac.sh` writes a byte-perfect WIFI file that the daemon accepts, but the chipset firmware on this M5 build holds the runtime WiFi MAC in an on-die cache that this repo's script cannot update. The cache is updated only through the Java app port [`flipphoneguy/mtk-imei-switcheroo-app`](https://github.com/flipphoneguy/mtk-imei-switcheroo-app), even though the app uses the same algorithm and the same on-device primitives (`mount -o remount,rw → cp → chmod 660 → chown root:system`). **For WiFi MAC changes on TIQ M5, install the app port APK and use it; for BT, IMEI, and WiFi *file-level* patching this repo's scripts work as on F21 Pro / F25.** Full investigation, ruled-out hypotheses, and reproduction steps: [`docs/tiq_m5_mac_live_findings.md`](docs/tiq_m5_mac_live_findings.md).
