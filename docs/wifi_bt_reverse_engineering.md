...
- **TIQ M5 (live-tested — partial)** — partition dumps analyzed in [`tiq_m5_offline_analysis.md`](tiq_m5_offline_analysis.md); live-device findings in [`tiq_m5_mac_live_findings.md`](tiq_m5_mac_live_findings.md). BT_Addr format / WIFI format / trailer-checksum algorithm all match F21 Pro byte-for-byte; `mac_tool.py` round-trips full `nvdata.bin` byte-identically. **BT MAC patching via `live_patch_mac.sh` works end-to-end on TIQ M5 hardware** — verified across reboots: file persists, daemon validates, `settings get secure bluetooth_address` returns the patched MAC. **WiFi MAC patching writes a byte-perfect file the daemon accepts, but on this M5 build the chipset firmware holds the runtime WiFi MAC in an on-die cache that this repo's flow cannot update** — neither `live_patch_mac.sh` nor an offline-patched `fastboot flash nvdata` changes the runtime MAC the AP sees. The Java app port [`flipphoneguy/mtk-imei-switcheroo-app`](https://github.com/flipphoneguy/mtk-imei-switcheroo-app) (whose `MacCrypto.java` is bit-for-bit equivalent to `mac_tool.compute_checksum` and whose `RootRunner.replaceFile` runs the same `mount remount,rw → cp → chmod 660 → chown root:system` primitives) does update the cache; we could not identify the differentiator that lets the app's `cp` reach the chipset cache while the script's identical `cp` does not. The recommended workflow on TIQ M5 is `live_patch_mac.sh` for BT and the app port for WiFi runtime; full investigation in `tiq_m5_mac_live_findings.md`.

- **F25 (offline + hardware confirmed via this repo's `live_patch_mac.sh` and via the Java app port)** — partition dumps analyzed in [`f25_offline_analysis.md`](f25_offline_analysis.md). BT_Addr format identical to F21 Pro. WIFI format uses **header `01 00 09 00` instead of `01 00 08 00`** at offset 2; trailer-checksum algorithm is the same. `mac_tool.py` was extended (`WIFI_HDR_VARIANTS = (b'\x01\x00\x08\x00', b'\x01\x00\x09\x00')`) to accept both; full nvdata.bin round-trip is byte-identical. Hardware patching subsequently confirmed both via `live_patch_mac.sh` directly and via end-user usage of `flipphoneguy/mtk-imei-switcheroo-app`. The per-device byte at offset 2 (08 vs 09) is what changes across the WiFi-firmware family — the on-disk format and AP-side validation logic are otherwise the same.
