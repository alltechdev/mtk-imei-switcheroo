...
> **Status — offline-compatible after a small mac_tool.py extension:** F25's BT_Addr file uses the exact same format and trailer-checksum algorithm as F21 Pro and TIQ M5. F25's WIFI file uses a **different header magic** at offset 2: `01 00 09 00` instead of the F21 Pro / TIQ M5 `01 00 08 00`. The trailer-checksum algorithm is the same (verified by 3 valid F25 WIFI copies all matching). `mac_tool.py` was extended in the same change-set as this doc to accept both header variants — `WIFI_HDR_VARIANTS = (bytes.fromhex('01000800'), bytes.fromhex('01000900'))`. After the extension, both `read` and `write` recognize F25 WIFI records (verified — full nvdata round-trip is byte-identical). F25 hardware verification is still required before claiming "live-device supported", per the project rule, but offline-tool compatibility is complete.
...
**For BT_Addr patching on F25**: every offline check passes, identical to F21 Pro / TIQ M5. `mac_tool.py write` round-trips byte-identically. End-to-end on F25 hardware still required before claiming "live-device supported", per the project rule, but offline confidence is high.

**For WIFI patching on F25**: now supported by `mac_tool.py` after the `WIFI_HDR_VARIANTS` extension. `mac_tool.py read tmp/f25/nvdata.bin` finds 2 WIFI copies; `mac_tool.py write … --wifi …` patches them; full-image round-trip is byte-identical (covered by `tests/mac_tool_edge_cases.sh`'s "F25 nvdata full round-trip" assertion). End-to-end on F25 hardware still required before claiming "live-device supported".
